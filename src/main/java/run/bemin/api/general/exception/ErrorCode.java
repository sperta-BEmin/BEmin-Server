...
public enum ErrorCode {
  ...
  AUTH_ACCESS_DENIED(HttpStatus.FORBIDDEN.value(), "A001", "접근이 거부되었습니다.") // 인증/인가 관련 오류
  ,
  ...
  CATEGORY_NOT_FOUND(HttpStatus.NOT_FOUND.value(), "CC001", "해당 카테고리를 찾을 수 없습니다.") // Category (카테고리 관련 오류)
  ,
  ...
  INVALID_INPUT_VALUE(HttpStatus.BAD_REQUEST.value(), "C001", "잘못된 입력값입니다.") // Common
  ,
  ...
  ORDER_NOT_FOUND(HttpStatus.NOT_FOUND.value(), "P001", "주문이 존재하지 않습니다.") // Payment Error Code (결제 관련 오류)
  ,
  ORDER_NOT_FOUND2(HttpStatus.NOT_FOUND.value(), "OR001", "주문이 존재하지 않습니다.") // Order (주문 관련 오류)
  ,
  ...
  PRODUCT_NOT_FOUND(HttpStatus.NOT_FOUND.value(), "PR001", "상품을 찾을 수 없습니다.") // Product (상품 관련 오류)
  ,
  SIGNIN_INVALID_CREDENTIALS(HttpStatus.UNAUTHORIZED.value(), "L002", "아이디 또는 비밀번호가 올바르지 않습니다."),
  SIGNIN_UNAUTHORIZED_USER(HttpStatus.UNAUTHORIZED.value(), "L001", "인증되지 않은 사용자입니다.") // Signin (로그인 관련 오류)
  ,
  SIGNUP_DUPLICATE_EMAIL(HttpStatus.BAD_REQUEST.value(), "S001", "이미 존재하는 이메일입니다.") // Signup (회원가입 관련 오류)
  ,
  ...
  STORE_ALREADY_DELETE(HttpStatus.FORBIDDEN.value(), "ST011", "이미 삭제된 가게입니다."),
  ...
  STORE_NOT_FOUND(HttpStatus.NOT_FOUND.value(), "ST001", "해당 가게를 찾을 수 없습니다.") // Store (가게 관련 오류)
  ,
  ...
  USER_PAGE_INDEX_INVALID(HttpStatus.BAD_REQUEST.value(), "U001", "페이지 인덱스는 0보다 작을 수 없습니다.") // User (유저 관련 오류)
  ,
